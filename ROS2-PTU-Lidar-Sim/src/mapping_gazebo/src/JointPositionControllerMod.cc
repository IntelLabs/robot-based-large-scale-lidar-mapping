...
#include "mapping_gazebo/JointPositionControllerMod.hh"
...
class gz::sim::systems::JointPositionControllerModPrivate
{
  ...
  public: OperationMode mode = OperationMode::ABS;
};

//////////////////////////////////////////////////
JointPositionControllerMod::JointPositionControllerMod()
  : dataPtr(std::make_unique<JointPositionControllerModPrivate>())
{
}

//////////////////////////////////////////////////
void JointPositionControllerMod::Configure(const Entity &_entity,
    ...
    EventManager &/*_eventMgr*/)
{
  ...
  {
    gzerr << "JointPositionControllerMod plugin should be attached to a model "
  ...
  if (_sdf->HasElement("use_velocity_commands"))
  {
    ...
    {
      this->dataPtr->mode =
        JointPositionControllerModPrivate::OperationMode::ABS;
    }
  }
  ...
  if (this->dataPtr->useActuatorMsg)
  {
    this->dataPtr->node.Subscribe(topic,
      &JointPositionControllerModPrivate::OnActuatorPos,
      this->dataPtr.get());

    gzmsg << "JointPositionControllerMod subscribing to Actuator messages on ["
      << topic << "]" << std::endl;
  }
  else
  {
    this->dataPtr->node.Subscribe(topic,
      &JointPositionControllerModPrivate::OnCmdPos,
      this->dataPtr.get());

    gzmsg << "JointPositionControllerMod subscribing to Double messages on ["
      << topic << "]" << std::endl;
  }

  gzdbg << "[JointPositionControllerMod] system parameters:" << std::endl;
...
//////////////////////////////////////////////////
void JointPositionControllerMod::PreUpdate(
    const UpdateInfo &_info,
    EntityComponentManager &_ecm)
{
  GZ_PROFILE("JointPositionControllerMod::PreUpdate");
  ...
  if (!this->dataPtr->model.Valid(_ecm))
  {
    gzwarn << "JointPositionControllerMod model no longer valid. "
  ...
  if (this->dataPtr->jointIndex >= jointPosComp->Data().size())
  {
    ...
    {
      gzerr << "[JointPositionControllerMod]: Detected an invalid <joint_index> "
...
//////////////////////////////////////////////////
void JointPositionControllerModPrivate::OnCmdPos(const msgs::Double &_msg)
{
  std::lock_guard<std::mutex> lock(this->jointCmdMutex);
  this->jointPosCmd = _msg.data();
}

void JointPositionControllerModPrivate::OnActuatorPos(const msgs::Actuators &_msg)
{
  std::lock_guard<std::mutex> lock(this->jointCmdMutex);
...
GZ_ADD_PLUGIN(JointPositionControllerMod,
                    System,
                    JointPositionControllerMod::ISystemConfigure,
                    JointPositionControllerMod::ISystemPreUpdate)

GZ_ADD_PLUGIN_ALIAS(JointPositionControllerMod,
                          "gz::sim::systems::JointPositionControllerMod")